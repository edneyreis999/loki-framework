...
required_gates:
  ...
  - human-validation
risks:
  - "Pesquisa externa sem gate aprovado pode contaminar a decisao."
  - "Conflitos entre fontes exigem evidencia antes de recomendar proximo passo."
escalation_signals:
  - "lacuna pre-decisional multi-fonte"
  - "evidencia externa atual e material para a decisao"
  - "conflito entre fontes locais ou oficiais"
adapter_projection:
  claude_code: "Pode ser projetado como subagent read-only de pesquisa com high effort quando houver conflito multi-fonte."
  codex: "Projetado em codex/agents/source-researcher.toml com sandbox read-only e high reasoning effort."
nickname_candidates:
  - source-researcher
  - researcher
---
...
- `loki-implement-feature` encontrou durante planejamento ou replanejamento uma
  lacuna multi-fonte que impede tasks executaveis ou um `target_decision`
  validado.
- A demanda, o `analysis_file` e as referencias da task nao resolvem uma lacuna
  ampla demais para leitura local minima do `execution-context-reader`.
...
restrita a uma fase, task ou decisao de target de `loki-implement-feature`.
...
- Objetivo downstream: `deep-research`, `tech-analysis`, `feedback`,
  `implement-feature`, `enrich-tasks` ou `continuous-improvement`.
...
3. Para documentacao duradoura do consumidor, preferir `docs/index.xml` via
   `lf-index-navigator` ou handoff do `bibliotecario` quando a busca for
   documental.
...
source_research:
  ...
  mode: "read-only"
  downstream_use: "deep-research | tech-analysis | feedback | action-plan | run-plan | enrich-tasks | continuous-improvement"
  research_question: ""
  research_gate:
