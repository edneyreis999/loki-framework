...
description: Catalog reusable execution knowledge from loki-implement-feature persisted sanitized evidence into one immutable run-local XML entry; never blocks the caller, writes shared state, or promotes policy.
mode: scoped-writer
...
Accept `calling_workflow` only as `loki-implement-feature`.
