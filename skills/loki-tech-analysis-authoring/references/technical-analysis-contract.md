...
on their evidence, risks and validators. Use provider-neutral fields; cite
concrete provider models only in a central projection reference when needed.
When running inside the package source, `docs/model-effort-guidance.md` may be
used as an optional central reference, not as an installed-skill runtime
dependency.
