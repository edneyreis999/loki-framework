...
8. Add a downstream execution profile so `loki:run-plan` can choose effort and
   handoffs without relying on chat memory.
...
Action plans generated by `loki:generate-action-plan` are transient artifacts,
but they are an explicit high-effort exception because they control future
execution. Use provider-neutral `model_class` and effort fields; do not
duplicate concrete model IDs in every plan. When running inside the package
source, `docs/model-effort-guidance.md` may be used as an optional central
reference, not as an installed-skill runtime dependency.
