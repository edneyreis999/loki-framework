...
description: Plan and implement a software or game feature from a non-empty demand and a readable Markdown technical analysis in one resumable invocation, with validated targets, DAG execution, per-task acceptance evidence, and a truthful terminal dashboard with manual tests.
...
when_to_use:
  ...
  - "Use when execution needs exact target provenance, task acceptance criteria, deterministic or independent validation, retry, resume, and a manual-test dashboard."
