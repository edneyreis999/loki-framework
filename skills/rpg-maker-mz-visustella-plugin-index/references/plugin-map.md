...
| Narrative And Presentation | Message Core, Visual Novel Picture Busts | text codes, choices, macros, word wrap, localization, picture busts, message and picture commands | message-core-localization, events-presentation, plugin commands, parameters |
...
| "localization", "language switching", `Localization:struct`, `LangFiletype:str`, `CsvFilename:str`, `TsvFilename:str`, `\tl{...}`, `\translate{...}`, `\loc{...}` | Message Core localization | `rpg-maker-mz-visustella-message-core-localization` when available; also `rpg-maker-mz-plugin-workflow` before `js/plugins.js` writes and `rpg-maker-mz-data-json` before data writes |
...
| message, text code, picture bust, visual novel, DragonBones, event movement, options, save, debugger | events/presentation | `rpg-maker-mz-visustella-events-presentation` when available; use `rpg-maker-mz-visustella-message-core-localization` for Message Core language tables or `\tl{...}` coverage |
